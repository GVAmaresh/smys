import Heading from "../Font/Heading";

export default function ProgramTemplate() {
  return (
    <>
      <Heading text="Programmes" />
      <div
        className="m-10 md:m-64 md:mt-10 md:mb-10 p-10 mt-10 mb-10 border-2"
        style={{
          borderBottomRightRadius: "50px",
          borderTopLeftRadius: "50px",
        }}
      >
        <li>
          ಅನ್ನಪೂರ್ಣ ನಿಧಿ: ಈ ನಿಧಿಯು ವಿದ್ಯಾರ್ಥಿಗಳಿಗೆ ಪೌಷ್ಟಿಕ ಆಹಾರವನ್ನು ಒದಗಿಸಲು
          ಸಂಪನ್ಮೂಲಗಳನ್ನು ಸಂಗ್ರಹಿಸುವ ಗುರಿಯನ್ನು ಹೊಂದಿದೆ, ಅವರು ನಿಲಯದಲ್ಲಿದ್ದಾಗ ಆಹಾರದ
          ಲಭ್ಯತೆಯನ್ನು ಖಚಿತಪಡಿಸುತ್ತದೆ.
        </li>
        <br />
        <li>
          ವಿದ್ಯಾರ್ಥಿವೇತನ ನಿಧಿ: ವಿದ್ಯಾರ್ಥಿವೇತನ ನಿಧಿಯು ಆರ್ಥಿಕವಾಗಿ ಅಗತ್ಯವಿರುವ
          ವಿದ್ಯಾರ್ಥಿಗಳಿಗೆ ಅವರ ಶಿಕ್ಷಣ ಅಥವಾ ಹಾಸ್ಟೆಲ್ ವಸತಿಗಾಗಿ ಆರ್ಥಿಕ ನೆರವು ನೀಡುವ
          ಮೂಲಕ ಸಹಾಯ ಮಾಡುತ್ತದೆ. ಇದು ಅರ್ಹ ವಿದ್ಯಾರ್ಥಿಗಳಿಗೆ ಹಣಕಾಸಿನ ನಿರ್ಬಂಧಗಳಿಲ್ಲದೆ
          ತಮ್ಮ ಅಧ್ಯಯನವನ್ನು ಮುಂದುವರಿಸಲು ಸಹಾಯ ಮಾಡುತ್ತದೆ.
        </li>
        <br />
        <li>
          ಕಟ್ಟಡ ನಿಧಿ: ಕಟ್ಟಡ ನಿಧಿಗೆ ನೀಡುವ ಕೊಡುಗೆಗಳನ್ನು ಸಂಸ್ಥೆಯ ಮೂಲಸೌಕರ್ಯವನ್ನು
          ಹೆಚ್ಚಿಸಲು ಬಳಸಲಾಗುತ್ತದೆ
        </li>
        <br />
        <li>
          ವಿಶೇಷ ಆಹಾರ ಪ್ರಾಯೋಜಕತ್ವ: ನಿರ್ದಿಷ್ಟ ದಿನಗಳಲ್ಲಿ ವಿದ್ಯಾರ್ಥಿಗಳಿಗೆ ವಿಶೇಷ
          ಊಟವನ್ನು ಪ್ರಾಯೋಜಿಸುವುದು ಅವರು ತಮ್ಮ ನಿಯಮಿತ ಊಟವನ್ನು ಮೀರಿ ಪೌಷ್ಟಿಕ ಆಹಾರವನ್ನು
          ಪಡೆಯುತ್ತಾರೆ ಎಂದು ಖಚಿತಪಡಿಸುತ್ತದೆ. ಇದು ಆಚರಣೆಗಳು, ಹಬ್ಬಗಳು ಅಥವಾ ಇತರ
          ಸಂದರ್ಭಗಳಲ್ಲಿ ಇರಬಹುದು.
        </li>
        <br />
        <li>
          ಗ್ರಂಥಾಲಯ ನಿಧಿ: ಗ್ರಂಥಾಲಯ ನಿಧಿಗೆ ನೀಡುವ ದೇಣಿಗೆಯನ್ನು ಹಾಸ್ಟೆಲ್ ಗ್ರಂಥಾಲಯಕ್ಕೆ
          ಹೊಸ ಪುಸ್ತಕಗಳು, ಶೈಕ್ಷಣಿಕ ಸಾಮಗ್ರಿಗಳು ಮತ್ತು ಸಂಪನ್ಮೂಲಗಳನ್ನು ಖರೀದಿಸಲು
          ಬಳಸಲಾಗುತ್ತದೆ. ಇದು ವಿದ್ಯಾರ್ಥಿಗಳ ಕಲಿಕೆಯ ಅನುಭವಗಳನ್ನು ಶ್ರೀಮಂತಗೊಳಿಸುತ್ತದೆ.
        </li>
        <br />
        <li>
          ಇತರ ದೇಣಿಗೆಗಳು: ಈ ವರ್ಗವು ದಾನಿಗಳಿಗೆ ಸಂಸ್ಥೆಯೊಳಗಿನ ಯಾವುದೇ ನಿರ್ದಿಷ್ಟ ಕಾರಣ
          ಅಥವಾ ಅಗತ್ಯಕ್ಕೆ ಕೊಡುಗೆ ನೀಡಲು ಅನುವು ಮಾಡಿಕೊಡುತ್ತದೆ. ಇದು ಉಪಕರಣಗಳು, ಪಠ್ಯೇತರ
          ಚಟುವಟಿಕೆಗಳು ಅಥವಾ ಇತರ ಅಗತ್ಯ ಅವಶ್ಯಕತೆಗಳನ್ನು ಒಳಗೊಂಡಿರಬಹುದು.
        </li>
        <br />

        <li>
          ದಾನ ಮಾಡಿದ ಮೊತ್ತವನ್ನು ದಾನಿ ನಿರ್ದಿಷ್ಟಪಡಿಸಿದ ಉದ್ದೇಶಕ್ಕಾಗಿ ಪ್ರತ್ಯೇಕವಾಗಿ
          ಹಂಚಿಕೆ ಮಾಡಲಾಗುತ್ತದೆ ಎಂಬುದನ್ನು ನೆನಪಿಡಿ.
        </li>
      </div>
    </>
  );
}
